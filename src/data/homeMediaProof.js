/**
 * Homepage social-proof media strip — sourced from wps.com
 * (Trusted by Experts / KOL · PR&Media · G2 Users).
 */

export const HOME_MEDIA_PROOF_TABS = [
  {
    id: 'kol',
    items: [
      {
        id: 'kevin-stratvert',
        author: 'Kevin Stratvert',
        quote:
          'A powerful Microsoft Office alternative with familiar design, full compatibility, and built-in PDF tools — trusted by millions worldwide.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/fb527d82afdd95b7211dd3f2c78b4d99.jpg',
        duration: '14:14',
        youtubeId: '9IlVkGulqAQ',
      },
      {
        id: 'michael-horn',
        author: 'Michael Horn',
        quote:
          'Designed to feel instantly familiar, with a clean interface and a smooth, easy-to-use experience.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/eed55eb0ec1867345eb873561984e45c.jpg',
        duration: '08:28',
        youtubeId: 'suPR7Qu4-kM',
      },
      {
        id: 'brett-in-tech',
        author: 'Brett In Tech',
        quote:
          'Excellent compatibility with Office formats, plus 100,000+ templates and a full suite of tools — all available across desktop and mobile.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/8adc4faad73760392b15b229f1de8917.jpg',
        duration: '07:04',
        youtubeId: 'HSwY9jvanPM',
      },
      {
        id: 'bogtechs',
        author: 'Bogtechs',
        quote:
          'Brings a desktop-like Office experience to mobile — familiar, easy to use, and powerful in one all-in-one app.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/66a24926ef650b59cde7204a1284aa22.jpg',
        duration: '14:06',
        youtubeId: 'PJIFvA1Hkcs',
      },
      {
        id: 'levelup-id',
        author: 'Levelup iD',
        quote:
          'Create, edit, and manage all your documents in one place — with templates, PDF tools, and full cross-platform support.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/c1f0a40c7f6f039e68a3009bc6db18dc.jpg',
        duration: '08:53',
        youtubeId: '7i3XIv5sMZc',
      },
      {
        id: 'ai-border',
        author: 'AI BORDER',
        quote:
          'With powerful AI tools, you can generate content, translate full documents instantly, summarize long files, and even chat with your PDFs for quick insights.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/552cf6216d3f6ef9b385aa41ac67c7b9.jpg',
        duration: '04:05',
        youtubeId: '5yjgEBHyRrk',
      },
      {
        id: 'hoc-vien',
        author: 'Học Viện',
        quote:
          'With a familiar interface, powerful AI features for writing and translation, and smooth performance across devices, WPS is a smart and efficient way to get your work done.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/3439fe9a87d9c7cfa0882f614a4501b8.jpg',
        duration: '09:16',
        youtubeId: 'Y4S2INhEIfE',
      },
      {
        id: 'mazthertutoriales',
        author: 'Mazthertutoriales',
        quote:
          'Fast, lightweight, and easy to use, it’s one of the best choices for creating and editing documents, spreadsheets, and presentations without hassle.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/1c201460fb2177c4c1f44df32ed572ea.jpg',
        duration: '08:02',
        youtubeId: 'tbc0eh4QDw0',
      },
    ],
  },
  {
    id: 'pr',
    items: [
      {
        id: 'techradar',
        author: 'TechRadar',
        quote: 'A full office suite that\'s completely free and genuinely impressive.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/6ac44531b242c87961a753239ab355fc.jpg',
        rating: 5,
      },
      {
        id: 'cnet',
        author: 'CNet',
        quote: 'An all-in-one office app with speedy performance.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/fea99d4494a2418409feae81662e5e4b.jpg',
        rating: 4,
      },
      {
        id: 'capterra',
        author: 'Capterra',
        quote:
          'WPS Office is a free, small and fast office suite, that provides you three powerful applications known as Writer, Spreadsheets and Presentation.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/d9666a190595509612dcd083896b0b26.jpg',
        rating: 5,
      },
      {
        id: 'toms-guide',
        author: "Tom's Guide",
        quote: 'The best PDF reader apps for Android and iOS let you read important documents on the go.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/a29baa299f6dc015cee1c2eb99029d47.jpg',
        rating: 5,
      },
      {
        id: 'android-authority',
        author: 'Android Authority',
        quote: 'WPS is among the best free office apps.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/87cad798b62afa9224dcf8f4e17ccb03.jpg',
        rating: 5,
      },
      {
        id: 'macworld',
        author: 'Macworld',
        quote: 'Full-featured Microsoft-style productivity suite for less.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/e309e9fac309c780b144f8caa1ca637b.jpg',
        rating: 3.5,
      },
      {
        id: 'pcworld',
        author: 'PCWorld',
        quote: 'WPS Office provides the most Office-like experience of any alternative.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/c20e35a9c3dbdf204cf9c2bb787070f7.jpg',
        rating: 5,
      },
      {
        id: 'laptop-mag',
        author: 'Laptop Mag',
        quote:
          'WPS Office is the most attractive Office 365 alternative, thanks to its customizable interface and other unique features.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/36ae98c4babfe843458a5803b11e178f.jpg',
        rating: 5,
      },
    ],
  },
  {
    id: 'g2',
    items: [
      {
        id: 'bhargav-a',
        author: 'Bhargav A.',
        quote: 'Great Online Document Sharing, Slow Startup',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/0cd3d410ccab322ca6298317a5760289.jpg',
        rating: 5,
      },
      {
        id: 'lokesh-n',
        author: 'Lokesh N.',
        quote: 'Amazing documentation tool for PC & Phone!!!',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/bb534e8a30fc774b4ba7962e7cae4f49.jpg',
        rating: 4,
      },
      {
        id: 'harinadhasankar-m',
        author: 'harinadhasankar m.',
        quote: 'Feature-Rich, User-Friendly Alternative to Microsoft Office',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/369dff779d8df8dc220a3dd30e22f71a.jpg',
        rating: 5,
      },
      {
        id: 'neha-y',
        author: 'Neha Y.',
        quote: 'Perfect tool for editing, customizing and converting documents.',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/0b1834d6b9b43499932153e05072263f.jpg',
        rating: 4.5,
      },
      {
        id: 'bilal-d',
        author: 'BILAL D.',
        quote:
          'It is all in one software for carrying out different tasks of opening,editing and configuring files',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/78d95328c936971c48d332241b025855.jpg',
        rating: 4.5,
      },
      {
        id: 'shivam-m',
        author: 'Shivam M.',
        quote: 'Best alternative to Ms-office suite',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/f568191688d3745a7ebc03e061a63c3f.jpg',
        rating: 5,
      },
      {
        id: 'nayanjyoti-g',
        author: 'Nayanjyoti G.',
        quote: 'Best office suite I have used so far',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/ad953909fb6f518a81503a97908269ba.jpg',
        rating: 5,
      },
      {
        id: 'sudhira-p',
        author: 'Sudhira P.',
        quote: 'Have All Features To Work On Different Type Of Files',
        thumb:
          'https://res-academy.cache.wpscdn.com/upload/seo-engine-admin/2026-03-24/aa4282279d103df5e23049421aa7d53e.jpg',
        rating: 4.5,
      },
    ],
  },
]
